...
export default function CallPage(){
  ...
  const setupPC=async()=>{
    ...
    pcRef.current=pc;
    try{
      const st=await navigator.mediaDevices.getUserMedia(callType==="video"?{video:true,audio:true}:{audio:true});
      streamRef.current=st;
      st.getTracks().forEach(t=>pc.addTrack(t,st));
      if(localRef.current&&callType==="video"){ localRef.current.srcObject=st; localRef.current.muted=true; }
    }catch{
      try{
    ...
    pc.onicecandidate=(e)=>{ if(e.candidate) sockRef.current?.emit("webrtc:ice",{roomId,candidate:e.candidate,fromId:userId}); };
    return pc;
  };

  const doWebRTC=async(init:boolean)=>{
    await setupPC();
    if(init){
      const of=await pcRef.current!.createOffer();
      await pcRef.current!.setLocalDescription(of);
      sockRef.current?.emit("webrtc:offer",{roomId,offer:of,fromId:userId});
    }
  };

  const cleanup=()=>{ streamRef.current?.getTracks().forEach(t=>t.stop()); pcRef.current?.close(); streamRef.current=null; pcRef.current=null; };

  const acceptCall=async()=>{
    sockRef.current?.emit("call:accept",{roomId:inc?.roomId||roomId,doctorId:userId,doctorName:userName});
    setStatus("connected");
    await doWebRTC(false);
    setInc(null);
  };
  ...
  const backUrl=role==="doctor"?"/doctor/dashboard":"/dashboard";
      ...
      {(status==="incoming"||inc)&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,background:"linear-gradient(180deg,#020D1A,#0D2040)"}}>
          ...
          <div style={{display:"flex",gap:40}}>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
              <button onClick={acceptCall} style={{width:68,height:68,borderRadius:"50%",background:"linear-gradient(135deg,#00C9A7,#0B6FCC)",border:"none",cursor:"pointer",fontSize:26,display:"flex",alignItems:"center",justifyContent:"center",animation:"ps 1.5s infinite"}}>📞</button>
      ...
      {status==="connected"&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",position:"relative"}}>
          ...
          {callType==="video"&&!vidOff&&(
            <video ref={localRef} autoPlay playsInline muted style={{position:"absolute",top:16,right:16,width:90,height:130,objectFit:"cover",borderRadius:12,border:"2px solid rgba(0,255,209,.3)",background:"#0D1B35"}}/>
          )}
          <div style={{position:"absolute",top:16,left:16,display:"flex",alignItems:"center",gap:7,padding:"6px 13px",borderRadius:100,background:"rgba(2,13,26,.8)"}}>
          ...
          <div style={{flexShrink:0,padding:"14px 20px 24px",background:"rgba(2,13,26,.9)",backdropFilter:"blur(20px)",display:"flex",alignItems:"center",justifyContent:"center",gap:16}}>
            <button onClick={toggleMic} style={{width:54,height:54,borderRadius:"50%",background:muted?"rgba(255,107,107,.2)":"rgba(255,255,255,.08)",border:"none",cursor:"pointer",fontSize:22,display:"flex",alignItems:"center",justifyContent:"center"}}>{muted?"🔇":"🎙️"}</button>
            {callType==="video"&&<button onClick={toggleCam} style={{width:54,height:54,borderRadius:"50%",background:vidOff?"rgba(255,107,107,.2)":"rgba(255,255,255,.08)",border:"none",cursor:"pointer",fontSize:22,display:"flex",alignItems:"center",justifyContent:"center"}}>{vidOff?"📷":"📹"}</button>}
            <button onClick={endCall} style={{width:64,height:64,borderRadius:"50%",background:"linear-gradient(135deg,#FF4B4B,#CC0000)",border:"none",cursor:"pointer",fontSize:26,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 24px rgba(255,75,75,.5)"}}>📵</button>
          </div>
        </div>
      )}
      ...
      {(status==="ended"||status==="rejected"||status==="error")&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
          ...
          <p style={{color:"rgba(232,244,255,.45)",fontSize:13,marginBottom:8}}>
            {status==="ended"?`Duration: ${fmt(dur)}`:status==="rejected"?"Doctor unavailable":errMsg||"Could not connect"}
          </p>
