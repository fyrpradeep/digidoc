...
const S = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .gc{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:14px;margin-bottom:11px;cursor:pointer;transition:all 0.3s}
  .gc:hover{border-color:rgba(0,255,209,0.18)}
  ...
  .bg{display:flex;align-items:center;justify-content:center;gap:8px;padding:11px;border-radius:13px;font-family:inherit;font-weight:600;font-size:12px;color:#00FFD1;border:1px solid rgba(0,255,209,0.22);background:rgba(0,255,209,0.05);cursor:pointer;transition:all 0.2s;text-decoration:none}
  ...
  .quick-btn:hover{border-color:rgba(0,255,209,0.22);transform:translateY(-2px)}
...
export default function PatientDashboard() {
  ...
  const connectSocket = (u: any) => {
    ...
    import("socket.io-client").then(({ io }) => {
      ...
      socket.on("connect", () => {
        socket.emit("register", { userId: u.id, role: "patient", name: u.name||"Patient" });
      });
    }).catch(() => {});
  };
  ...
  const callDoctor = (doctor: any, type: "video"|"audio") => {
    ...
    router.push(`/call/${roomId}?role=patient&userId=${u?.id||'p1'}&userName=${encodeURIComponent(u?.name||'Patient')}&doctorId=${doctor.id}&doctorName=${encodeURIComponent(doctor.name)}&callType=${type}`);
  };
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 18px"}} className="noscroll">
        ...
        {tab==="home"&&(
          <div style={{paddingTop:16,paddingBottom:16,animation:"fadeUp 0.5s ease"}}>
            ...
            <div style={{display:"flex",gap:10,marginBottom:18}}>
              {[{n:prescriptions.length,l:"Prescriptions",c:"#00FFD1"},{n:orders.length,l:"Orders",c:"#4DB8FF"},{n:doctors.length,l:"Doctors",c:"#A78BFA"}].map(s=>(
            ...
            {prescriptions.length===0&&!loading&&(
              <div style={{background:"linear-gradient(135deg,rgba(0,201,167,0.12),rgba(11,111,204,0.15))",border:"1px solid rgba(0,255,209,0.18)",borderRadius:18,padding:"18px",marginBottom:18}}>
                <p style={{fontSize:13,fontWeight:800,color:"#E8F4FF",marginBottom:4}}>👋 Welcome, {firstName}!</p>
                <p style={{color:"rgba(232,244,255,0.5)",fontSize:12,lineHeight:1.7,marginBottom:14}}>Connect to a doctor in under 2 minutes.</p>
            ...
            {loading?(
              <div style={{display:"flex",justifyContent:"center",padding:"24px 0"}}><span className="loader"/></div>
            ):doctors.length===0?(
              <div style={{textAlign:"center",padding:"28px 20px"}}>
                <p style={{fontSize:36,marginBottom:8}}>🩺</p>
                <p style={{fontWeight:700,fontSize:14,color:"#E8F4FF",marginBottom:4}}>No doctors online right now</p>
                <p style={{color:"rgba(232,244,255,0.38)",fontSize:12}}>Try AI chat for guidance</p>
              </div>
            ):(
              doctors.filter((d:any)=>d.isOnline||d.status==="approved").slice(0,3).map((d:any)=>(
                <div key={d.id} className="gc">
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <button className="bm" style={{flex:2,padding:"10px"}} onClick={()=>callDoctor(d,"video")}>📹 Video Call</button>
                    <button className="bg" style={{flex:1}} onClick={()=>callDoctor(d,"audio")}>📞 Audio</button>
                  </div>
                </div>
              ))
            )}
        ...
        {tab==="doctors"&&(
          <div style={{paddingTop:16,paddingBottom:16}}>
            <p style={{fontSize:10,fontWeight:700,color:"rgba(232,244,255,0.35)",textTransform:"uppercase",letterSpacing:1.2,marginBottom:14}}>All Doctors ({doctors.length})</p>
            {loading?<div style={{display:"flex",justifyContent:"center",padding:"32px 0"}}><span className="loader"/></div>
            :doctors.length===0?<div style={{textAlign:"center",padding:"32px"}}><p style={{fontSize:40,marginBottom:10}}>🩺</p><p style={{color:"rgba(232,244,255,0.38)",fontSize:13}}>No doctors yet</p></div>
            :doctors.map((d:any)=>(
              <div key={d.id} className="gc">
                <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:10}}>
                  <div style={{width:46,height:46,borderRadius:14,background:"rgba(0,255,209,0.07)",border:"1px solid rgba(0,255,209,0.14)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>👨‍⚕️</div>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}><p style={{fontWeight:700,fontSize:13,color:"#E8F4FF"}}>{d.name}</p>{d.isOnline&&<span className="livdot"/>}</div>
        ...
        {tab==="prescriptions"&&(
          <div style={{paddingTop:16,paddingBottom:16}}>
            <p style={{fontSize:10,fontWeight:700,color:"rgba(232,244,255,0.35)",textTransform:"uppercase",letterSpacing:1.2,marginBottom:14}}>Prescriptions ({prescriptions.length})</p>
            {loading?<div style={{display:"flex",justifyContent:"center",padding:"32px 0"}}><span className="loader"/></div>
            :prescriptions.length===0?(
              <div style={{textAlign:"center",padding:"32px 20px"}}>
                <p style={{fontSize:40,marginBottom:10}}>📋</p>
                <p style={{fontWeight:700,fontSize:15,color:"#E8F4FF",marginBottom:4}}>No prescriptions yet</p>
                <p style={{color:"rgba(232,244,255,0.38)",fontSize:12,marginBottom:18}}>Book a consultation first</p>
                <a href="/symptoms" className="bm" style={{display:"inline-flex",padding:"12px 22px",width:"auto"}}>🩺 Book Consultation</a>
              </div>
              ...
              <div key={rx.id} className="gc" onClick={()=>router.push(`/prescription/${rx.id}`)}>
                ...
                <p style={{color:"rgba(232,244,255,0.3)",fontSize:10,marginTop:3}}>{new Date(rx.createdAt).toLocaleDateString("en-IN")}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {tab==="profile"&&(
          <div style={{paddingTop:16,paddingBottom:16}}>
            <div style={{background:"linear-gradient(135deg,rgba(0,201,167,0.1),rgba(11,111,204,0.12))",border:"1px solid rgba(0,255,209,0.15)",borderRadius:18,padding:18,marginBottom:16,textAlign:"center"}}>
              ...
              <p style={{color:"rgba(232,244,255,0.35)",fontSize:11,marginTop:2}}>{user?.mobile?`+91 ${user.mobile}`:localStorage.getItem("digidoc_mobile")?`+91 ${localStorage.getItem("digidoc_mobile")}`:""}</p>
            </div>
            ...
            {[{icon:"👤",l:"Edit Profile",h:"/profile"},{icon:"📋",l:"Medical Records",h:"/records"},{icon:"💊",l:"My Orders",h:"/order"},{icon:"💳",l:"Payments",h:"/payments"},{icon:"❓",l:"Help",h:"/support"}].map(item=>(
