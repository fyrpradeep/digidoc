...
const S = `
  *{box-sizing:border-box;margin:0;padding:0}html,body{height:100%;overflow:hidden}
  ...
  .bm{display:flex;align-items:center;justify-content:center;gap:8px;padding:12px 18px;border-radius:13px;font-family:inherit;font-weight:700;font-size:13px;color:#fff;border:none;cursor:pointer;background:linear-gradient(135deg,#00C9A7,#0B6FCC);transition:all .2s}
...
export default function DoctorDashboard() {
  ...
  const connectSocket = (u: any) => {
    ...
    import("socket.io-client").then(({ io }) => {
      const s = io(`${URL}/call`, { transports: ["websocket","polling"], reconnectionAttempts: 5, timeout: 8000 });
      sockRef.current = s;
      const doctorId = u?.id || getMobile();
      s.on("connect", () => {
        setSockSt("on");
        s.emit("register", { userId: doctorId, role: "doctor", name: u?.name || "Doctor" });
        console.log("✅ Doctor live, ID:", doctorId);
        // Update online status in backend
        try {
          fetch(`${API}/doctors/me/online`, { method:"PUT", headers:{"Content-Type":"application/json",Authorization:`Bearer ${getToken()}`}, body: JSON.stringify({isOnline:true}) });
        } catch {}
      });
      s.on("disconnect", () => setSockSt("fail"));
      s.on("connect_error", () => setSockSt("fail"));
      s.on("call:incoming", (d: any) => {
        console.log("📞 Incoming:", d.patientName);
        setInc(d);
        try {
          ...
          [0,800,1600].forEach(delay => {
            ...
            o.start(ac.currentTime+delay/1000); o.stop(ac.currentTime+delay/1000+0.4);
          });
        } catch {}
      });
  ...
  const disconnectSocket = () => {
    ...
    sockRef.current = null; setSockSt("off");
    try { fetch(`${API}/doctors/me/online`, { method:"PUT", headers:{"Content-Type":"application/json",Authorization:`Bearer ${getToken()}`}, body: JSON.stringify({isOnline:false}) }); } catch {}
  };

  const toggleOnline = (v: boolean) => {
    setOnline(v);
    if (v) connectSocket(user || getUser());
  ...
  const reject = () => {
    sockRef.current?.emit("call:reject", { roomId: inc?.roomId, doctorId: user?.id||getMobile() });
    setInc(null);
  };
  ...
  if (isPending) return (
    <div style={{position:"fixed",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"#020D1A",fontFamily:"'Plus Jakarta Sans',sans-serif",color:"#E8F4FF",padding:24,textAlign:"center"}}>
      ...
      <p style={{color:"rgba(232,244,255,.5)",fontSize:14,lineHeight:1.8,marginBottom:24}}>Your doctor registration is pending admin approval.<br/>You'll be notified once approved.</p>
      <button onClick={()=>{localStorage.clear();window.location.href="/login";}} style={{padding:"12px 24px",borderRadius:13,background:"rgba(255,107,107,.07)",border:"1px solid rgba(255,107,107,.18)",color:"#FF6B6B",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>Sign Out</button>
    </div>
  );
  ...
  return (
    <div style={{position:"fixed",inset:0,display:"flex",flexDirection:"column",background:"#020D1A",fontFamily:"'Plus Jakarta Sans',sans-serif",color:"#E8F4FF"}}>
      ...
      <div className="ns" style={{flex:1,overflowY:"auto",padding:"0 18px"}}>
          ...
          <div style={{paddingTop:14,animation:"su .4s ease"}}>
            {!online ? (
              <div style={{textAlign:"center",padding:"50px 20px"}}>
                ...
                <p style={{color:"rgba(232,244,255,.4)",fontSize:13,marginBottom:20}}>Toggle Online to start receiving patient calls</p>
              ...
              <div style={{background:"rgba(0,255,209,.05)",border:"1px solid rgba(0,255,209,.2)",borderRadius:16,padding:20,textAlign:"center",marginTop:10}}>
                ...
                <p style={{color:"rgba(232,244,255,.45)",fontSize:13,lineHeight:1.7,marginBottom:10}}>Waiting for patient calls...</p>
                <p style={{color:"rgba(232,244,255,.3)",fontSize:11}}>Incoming call will appear as popup</p>
              </div>
            ) : sockSt==="connecting" ? (
              ...
              <div style={{textAlign:"center",padding:"36px 20px"}}>
                ...
                <p style={{color:"rgba(232,244,255,.4)",fontSize:12,marginBottom:14}}>Backend may be restarting. You are still marked online.</p>
                <button className="bm" style={{margin:"0 auto",width:"auto",padding:"10px 20px",fontSize:12}} onClick={()=>connectSocket(user||getUser())}>Retry Connection</button>
              </div>
            )}
          </div>
        )}
        ...
        {tab==="history" && (
          <div style={{paddingTop:14,textAlign:"center",padding:"50px 20px 14px"}}>
            <p style={{fontSize:36,marginBottom:10}}>📋</p>
            <p style={{fontWeight:700,fontSize:14,color:"#E8F4FF",marginBottom:4}}>No consultations yet</p>
            <p style={{color:"rgba(232,244,255,.38)",fontSize:12}}>Your history will appear here after calls</p>
          </div>
        )}
        ...
        {tab==="profile" && (
          <div style={{paddingTop:14}}>
            ...
            {[{i:"✏️",l:"Edit Profile",h:"/doctor/profile"},{i:"📅",l:"Schedule",h:"/doctor/profile"},{i:"❓",l:"Help & Support",h:"/support"}].map(it=>(
            ...
            <button onClick={()=>{disconnectSocket();localStorage.clear();window.location.href="/login";}} style={{width:"100%",padding:"13px",borderRadius:13,background:"rgba(255,107,107,.07)",border:"1px solid rgba(255,107,107,.18)",color:"#FF6B6B",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>Sign Out</button>
          </div>
        )}
      </div>
